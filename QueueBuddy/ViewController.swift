...
extension ViewController: SPTSessionManagerDelegate {
    func sessionManager(manager: SPTSessionManager, didFailWith error: Error) {
        if error.localizedDescription == "The operation couldn’t be completed. (com.spotify.sdk.login error 1.)" {
            presentAlertController(title: "Spotify Not Found", message: "Download the spotify app in the app store", buttonTitle: "Close")
        } else {
            presentAlertController(title: "Authorization Failed", message: "Confirm account information and try again", buttonTitle: "Close")
        }
    }

    func sessionManager(manager: SPTSessionManager, didRenew session: SPTSession) {
        presentAlertController(title: "Session Renewed", message: session.description, buttonTitle: "Close")
    }
